...
public class TimelapseService extends Service {
	...
	private EmotionServiceClient client;	private final String TAG = "TimelapseService";
...
flag++;
			...
			try {
				...
				0 Log.v( TAG, "::imageCallback: picture stored successfully as " + myname );

				File f = new File("/sdcard/floe.timelapse/");
				File[] list = f.listFiles();
			//	for(int i =0 ;i<list.length;i++) {
					Uri mImageUri = Uri.fromFile(list[0]);
		if(flag==1)			new RecognizeActivity().processWithAutoFaceDetection(mImageUri,getContentResolver());

				/*	Bitmap mBitmap = ImageHelper.loadSizeLimitedBitmapFromUri(
							mImageUri, getContentResolver());

					Gson gson = new Gson();		if (client == null) {
